...
public class PoseEstimator<T extends WheelPositions<T>> {
  ...
  public Rotation2d lastRotation2d = new Rotation2d();
  ...
  public void addVisionMeasurement(VisionObservation observation) {
    ...
    try {
      if (m_poseBuffer.getInternalBuffer().lastKey() - kBufferDuration > observation.timestamp()) {
        return;
      }
    ...
    Optional<Pose2d> sample = m_poseBuffer.getSample(observation.timestamp());

    if (sample.isEmpty()) {
        Logger.recordOutput("Odometry/SAMPLE IS EMPTY", true);
      return;
    }
...
//   }
  ...
  public Pose2d updateWithTime(double currentTimeSeconds, Rotation2d gyroAngle, SwerveDriveWheelPositions wheelPositions) {
    ...
    Twist2d twist = DriveConstants.kDriveKinematics.toTwist2d(lastWheelPositions, wheelPositions);
    lastWheelPositions = wheelPositions.copy();

    
    twist = new Twist2d(twist.dx, twist.dy, gyroAngle.minus(lastRotation2d).getRadians());
    Logger.recordOutput("Radian Change",gyroAngle.minus(lastRotation2d).getRadians());
    lastRotation2d = gyroAngle;
